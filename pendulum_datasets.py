...
def compare_datasets():
    ...
    if not cache_path.exists() or True:
    ...
    for i in range(X.shape[0]):
        print("---------------------------------")
        print(i, "Orig Y", Y[i])
        print(i, "VS rk1_simple", X_rk1_simple[i])
        print(i, "VS rk4_simple", X_rk4_simple[i])
        print(i, "VS rk1_long", X_rk1_long[i])
...
def gradient_rk4(X, pen_gen, h=0.01):
    steps = int(1/h)
...
def gradient_rk1(X, pen_gen, h=0.01):
    steps = int(1/h)
    # Initialize args.number initial positions:
    X_phy = np.zeros((steps + 1, *X.shape), dtype=np.float32)
    X_phy[0,...] = X
